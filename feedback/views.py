...
@require_GET
@login_required
@active_tab("feedback/handled")
def handled_feedback(request):
    query_set = HandledFeedback.objects.all().order_by("-pk")
    query = request.GET.get("q", None)
    if query:
        query_set = query_set.filter(Q(content__contains=query))
    table = HandledFeedbackTable(query_set)
    if query:
        table.empty_text = settings.NO_SEARCH_RESULTS
    count = len(Feedback.objects.all())
    RequestConfig(request, paginate={"per_page": settings.PAGINATION_PAGE_SIZE}).configure(table)
    return render(request, "handled.html", {
        'query': query,
        'table': table,
        'count': count if count > 0 else None
    })
